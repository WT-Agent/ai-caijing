<template>
  <div class="app-container">
    ...
    <header>
      <h1>{{ appTitle }}</h1>
      <p>宏观经济趋势解读 · 行业财报商业拆解 · 家庭资产配置理财</p>
    </header>
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="selector-group">
        <label class="selector-label">选择财经研判类型</label>
        <div class="style-selector">
          <button 
            v-for="atype in analysisTypeOptions" 
            :key="atype.value"
            class="style-option"
            :class="{ active: activeAnalysisType === atype.value }"
            @click="activeAnalysisType = atype.value"
          >
            {{ atype.label }}
          </button>
        </div>
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">投资风险偏好</label>
          <div class="style-selector">
            <button 
              v-for="risk in riskPreferenceOptions" 
              :key="risk"
              class="style-option"
              :class="{ active: selectedRiskPreference === risk }"
              @click="selectedRiskPreference = risk"
            >
              {{ risk }}
            </button>
          </div>
        ...
        <div class="selector-group">
          <label class="selector-label">专业分析视角</label>
          <div class="style-selector">
            <button 
              v-for="persp in perspectiveOptions" 
              :key="persp"
              class="style-option"
              :class="{ active: selectedPerspective === persp }"
              @click="selectedPerspective = persp"
            >
              {{ persp }}
            </button>
          </div>
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <label class="selector-label">输入财经话题、公司财报或理财配置需求</label>
          <div style="display: flex; gap: 0.5rem;">
            <button v-if="userInput" class="text-link-btn" @click="userInput = ''">清空输入</button>
            <button class="text-link-btn" @click="showFinanceTipsModal = true">资产配置与避坑指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
          placeholder="请简要描述您的财经研判课题或个人理财状况...（例如：分析在低利率与降息宏观环境下，家庭拥有 50 万可投资资金，如何按照“流动性/安全边际/长期增值”建构理财金字塔。）"
          style="min-height: 130px;"
        ></textarea>
        <div style="display: flex; justify-content: space-between; font-size: 0.75rem; color: var(--text-secondary);">
          <span>字符数: {{ userInput.length }} 字</span>
          <span>建议包含投资期限、资金规模、核心关切与风险承受底线</span>
        </div>
      </div>
    ...
    <section v-if="result || loading" class="glass-card">
      <div class="result-header">
        <span class="result-title">财经研判与投资理财报告</span>
        <div class="button-actions">
          <button v-if="result" class="icon-btn" @click="copyText">
            {{ copied ? '已复制报告全文' : '复制研判内容' }}
          </button>
    ...
    <section v-if="showHistory" class="glass-card" style="margin-top: 1rem;">
      <div class="result-header">
        <span class="result-title">本地财经与理财历史记录</span>
      ...
      <div v-else class="history-grid" style="display: flex; flex-direction: column; gap: 0.75rem; max-height: 320px; overflow-y: auto;">
        <div v-for="item in historyList" :key="item.id" class="history-item" style="padding: 1rem; background: rgba(0,0,0,0.2); border-radius: 10px; border: 1px solid var(--card-border);">
          <div style="display: flex; justify-content: space-between; font-size: 0.8rem; color: var(--text-secondary); margin-bottom: 0.4rem;">
            <span>{{ item.timestamp }} · [{{ item.analysisType }} / {{ item.riskPreference }}]</span>
          ...
          <div style="display: flex; gap: 0.5rem;">
            <button class="icon-btn" style="font-size: 0.75rem;" @click="applyHistory(item)">套用课题</button>
    ...
    <!-- 资产配置与避坑指南 Modal -->
    <div v-if="showFinanceTipsModal" class="modal-overlay" @click.self="showFinanceTipsModal = false">
      <div class="modal-content" style="max-width: 480px;">
        <h3>家庭资产配置与理财风控三铁律</h3>
        ...
        <div class="modal-scroll-area" style="text-align: left; font-size: 0.825rem;">
          <div v-for="(rule, idx) in financeRules" :key="idx" style="margin-bottom: 0.75rem; padding: 0.5rem 0.75rem; background: rgba(255,255,255,0.03); border-radius: 8px; border: 1px solid rgba(255,255,255,0.05);">
            <div style="color: var(--accent-color); font-weight: bold; margin-bottom: 0.2rem;">{{ rule.title }}</div>
            <div style="color: var(--text-primary); margin-bottom: 0.2rem;">配比建议: {{ rule.ratio }}</div>
            <div style="color: var(--text-secondary); font-size: 0.775rem;">避坑要点: {{ rule.avoid }}</div>
          </div>
        </div>
        <button class="modal-btn" style="margin-top: 1rem;" @click="showFinanceTipsModal = false">关闭</button>
      </div>
    </div>
    ...
    <div v-if="showShareGuide" class="modal-overlay" @click.self="showShareGuide = false">
      <div class="modal-content">
        <h3>分享财经分析与投资理财工具</h3>
        <p>扫码关注或将链接转发给关心家庭理财与商业趋势的朋友，理性投资避开风险陷阱。</p>
    ...
    <div v-if="showPrivacy" class="modal-overlay" @click.self="showPrivacy = false">
      <div class="modal-content">
        ...
        <div class="modal-text-content modal-scroll-area">
          <p>我们高度保密您的个人财务隐私。您在本工具中输入理财资金状况与投资偏好仅用于实时大模型生成，系统不会在云端存储您的资产数据。</p>
    ...
    <div v-if="showTerms" class="modal-overlay" @click.self="showTerms = false">
      <div class="modal-content">
        ...
        <div class="modal-text-content modal-scroll-area">
          <p>欢迎使用网腾无限 AI 财经分析与投资理财专家。本工具生成的分析结论仅供财商科普与资产配置思路参考。</p>
          <p>市场有风险，投资需谨慎。本报告不构成任何具体股票、基金或金融衍生品的买卖要约或保证。</p>
        </div>
...
// 配置参数
const appTitle = ref(appConfig.title || '网腾无限AI - 财经分析与投资理财专家');
...
const showFinanceTipsModal = ref(false);
...
const analysisTypeOptions = [
  { label: '宏观经济与政策趋势解读', value: '宏观经济与政策趋势解读' },
  { label: '行业赛道与财报商业拆解', value: '行业赛道与财报商业拆解' },
  { label: '个人与家庭资产配置理财', value: '个人与家庭资产配置理财' },
  { label: '自媒体爆款财经科普文案', value: '自媒体爆款财经科普文案' }
];
const activeAnalysisType = ref(analysisTypeOptions[0].value);

// 风险偏好与视角
const riskPreferenceOptions = ['保守稳健型', '平衡收益型', '积极进取型'];
const selectedRiskPreference = ref('保守稳健型');

const perspectiveOptions = ['机构研究员视角', '个人投资者视角', '企业财务总监CFO视角'];
const selectedPerspective = ref('个人投资者视角');

// 评估指标列表
const metricsList = [
  { key: 'dataAccuracy', label: '财务数据严谨度' },
  { key: 'macroInsight', label: '宏观洞察深刻度' },
  { key: 'riskWarningSensitivity', label: '风险防范预警度' },
  { key: 'assetAllocationRigor', label: '资产配置科学度' },
  { key: 'comprehensibility', label: '商业逻辑通俗度' }
];
...
// 理财配置法则
const financeRules = [
  { title: '应急准备金法则', ratio: '保留 3-6 个月月度开销的活期存款', avoid: '切忌全仓投入无法随时变现的重资产或定期' },
  { title: '核心与卫星配置', ratio: '80% 稳健底仓 + 20% 弹性进取视角', avoid: '切勿盲目加杠杆或借贷抄底' },
  { title: '能力圈与分散化', ratio: '不懂不投，跨资产类别 (债券/股票/大宗) 分散', avoid: '严禁把所有资金押宝在单一股票或虚拟资产' }
];

// 计算纯结果文本 (剔除打分标签 [CAIJING_SCORES])
const displayResultText = computed(() => {
  if (!result.value) return '';
  return result.value.replace(/\[CAIJING_SCORES\][\s\S]*?\[\/CAIJING_SCORES\]/g, '').trim();
});

// 解析打分标签
const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[CAIJING_SCORES\](.*?)\[\/CAIJING_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('caijing_history_records');
...
const saveHistory = () => {
  localStorage.setItem('caijing_history_records', JSON.stringify(historyList.value));
};
...
const addHistoryRecord = () => {
  const newItem: HistoryItem = {
    ...
    riskPreference: selectedRiskPreference.value,
...
const applyHistory = (item: HistoryItem) => {
  ...
  activeAnalysisType.value = item.analysisType;
  selectedRiskPreference.value = item.riskPreference;
...
const handleGenerate = async () => {
  ...
  try {
    const response = await fetch(apiEndpoint, {
      ...
      body: JSON.stringify({
        taskType: 'text',
        prompt: `任务指导: ${promptTopic.value}\n【研判类型】: ${activeAnalysisType.value}\n【风险偏好】: ${selectedRiskPreference.value}\n【分析视角】: ${selectedPerspective.value}\n【话题/需求描述】: ${userInput.value}`,
        style: activeAnalysisType.value
      })
    });
...
const handleApplyTemplate = (payload: { prompt: string; analysisType?: string; riskPreference?: string }) => {
  userInput.value = payload.prompt;
  if (payload.analysisType) activeAnalysisType.value = payload.analysisType;
  if (payload.riskPreference) selectedRiskPreference.value = payload.riskPreference;
  if (inputCardRef.value) {
    inputCardRef.value.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }
};
